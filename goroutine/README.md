...
### В чем отличия горутин от потоков?
1. Стоимость создания
   - Поток → создание тяжёлое (системный вызов, стек ~МБ).
   - Горутина → лёгкая, выделяется минимальный стек (~2 KB).
2. Контекстное переключение
   - Поток → переключение управляется ОС → дорого (системные прерывания).
   - Горутина → переключение управляется Go runtime → дешевле (в user space).
3. Планировщик
   - Потоки → планируются ядром ОС (preemptive scheduling).
   - Горутины → планируются Go runtime (M:N model):
     - M — потоки ОС,
     - P — процессоры (логические),
     - G — горутины.
4. Масштабируемость
   - Потоки → тысячи → перегруз по памяти и CPU.
   - Горутины → миллионы → без проблем (IO bound, сетевые сервисы).
   - Runtime сам решает, какая горутина будет выполнена на каком потоке. 
5. Синхронизация
   - Потоки → обычно работают через мьютексы/семафоры → сложно.
   - Горутины → есть каналы, context, select → более высокоуровневые средства коммуникации.
#### Как ответить на собесе
Горутины — это лёгкие задачи, управляемые рантаймом Go, а не ОС. Они используют динамический стек (начинается с 2 KB), тогда как потокам ОС выделяется сразу мегабайт. Планирование горутин реализовано внутри Go по модели M:N (много горутин к малому числу потоков), что делает переключение дешёвым и позволяет запускать миллионы горутин одновременно. В отличие от потоков, горутины тесно интегрированы с каналами и context, что упрощает синхронизацию.
