...
Цепочки (Chaining): Каждый элемент с одинаковым хешом хранится в списке (или другой структуре данных) в том же индексе.
Открытая адресация (Open Addressing): Если два ключа хешируются в один индекс, то поиск "пустого" места для второго элемента будет происходить в другом месте таблицы.
